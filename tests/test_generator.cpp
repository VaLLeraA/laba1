...
struct TestCase {
    ...
    TestCase(const std::string &testName, bool isNegative = false)
        : name(testName), templateFile((fs::path("test_cases") / testName / "template.txt").string()), dataFile((fs::path("test_cases") / testName / "data.dat").string()), expectedFile((fs::path("test_cases") / testName / "expected.txt").string()), isNegativeTest(isNegative) {}
};
